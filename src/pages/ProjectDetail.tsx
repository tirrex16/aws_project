...
export default function ProjectDetail() {
  ...
  if (!project) {
    return (
      <>
        <Helmet>
          <title>Project Not Found | Tirrex Studio</title>
  ...
  const canonicalUrl = `https://tirrex.com/projects/${project.id}`
    ...
    '@graph': [
      {
        ...
        creator: {
          '@type': 'Person',
          '@id': 'https://tirrex.com/#person',
          name: 'Mohammed Winston',
        },
      },
      {
        '@type': 'BreadcrumbList',
        itemListElement: [
          { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://tirrex.com/' },
          { '@type': 'ListItem', position: 2, name: 'Projects', item: 'https://tirrex.com/#work' },
  ...
  return (
    <>
      <Helmet>
        <title>{project.name} | Tirrex Studio</title>
        ...
        <meta property="og:title" content={`${project.name} | Tirrex Studio`} />
        <meta property="og:description" content={project.tagline} />
        <meta property="og:image" content={project.heroImg} />
        <meta property="og:image:alt" content={`${project.name} — project by Mohammed Winston`} />
        <meta property="og:site_name" content="Tirrex Studio" />

        {/* Twitter */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${project.name} | Tirrex Studio`} />
        <meta name="twitter:description" content={project.tagline} />
        <meta name="twitter:image" content={project.heroImg} />
        <meta name="twitter:image:alt" content={`${project.name} — project by Mohammed Winston`} />
      ...
      <div className="pt-[140px] pb-[120px] max-w-[1560px] mx-auto px-10" ref={topRef}>
        ...
        <div className="mb-[120px] rounded-[24px] overflow-hidden reveal">
          <img
            src={project.heroImg}
            alt={`${project.name} — hero image by Mohammed Winston`}
        ...
        {project.sections.map((sec, idx) => (
          <section key={idx} className="mb-[120px] reveal" aria-label={sec.label}>
        ...
        {/* ── Related projects ────────────────────── */}
        <section className="pt-[120px] border-t border-[#e8e8e8] reveal" aria-labelledby="related-heading">
          <div className="flex justify-between items-end mb-10 gap-6">
            <div>
              <h2 id="related-heading" className="text-[clamp(2.25rem,6vw,4rem)] font-extrabold tracking-[-0.045em] leading-none mb-3">
                Latest Projects.
              </h2>
            ...
            {related.map((rp, i) => (
              <Link key={i} to={`/projects/${rp.id}`} className="block cursor-pointer overflow-hidden rounded-[18px] bg-[#f5f5f5] border border-[#e8e8e8] no-underline text-[#0f0f0f] group transition-all duration-[400ms] hover:bg-[#0f0f0f] hover:border-[#0f0f0f] reveal reveal-d1">
                <div className="p-2.5 pb-0">
                  <img
                    src={rp.img}
                    alt={`${rp.name} — ${rp.type} project by Mohammed Winston`}
                ...
                <div className="flex justify-between items-start p-[14px_16px_16px]">
                  <div>
                    <h3 className="text-[0.9375rem] font-semibold tracking-[-0.02em] group-hover:text-white transition-colors duration-[400ms]">{rp.name}</h3>
